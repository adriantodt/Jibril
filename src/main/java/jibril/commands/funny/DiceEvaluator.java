...
public class DiceEvaluator {
    ...
    private final ShadowDice dice = new ShadowDice();
    ...
    private double parseConst(String key) {
        ...
        if (DICE.matcher(key).matches()) {
            ...
            int rolls = Math.min((parts[0] == null || parts[0].isEmpty() ? 1 : Integer.parseInt(parts[0])), MAX_ROLLS);
            int sides = Math.min(Integer.parseInt(parts[1]), MAX_SIDES);

            return IntStream.range(0, rolls)
                .parallel()
                .mapToLong(operand -> dice.roll(sides))
                .sum();
        }
