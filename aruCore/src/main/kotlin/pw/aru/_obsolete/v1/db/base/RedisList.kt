...
class RedisList<E>(
    ...
    override val size: Int
        get() = db.conn.sync().llen(hash).toInt()

    override fun isEmpty(): Boolean = size == 0

    override fun get(index: Int): E = serializer.unserialize(db.conn.sync().lindex(hash, index.toLong()))
    ...
    private fun all(redis: RedisCommands<String, String>): MutableList<String> {
        return redis.lrange(hash, 0, -1)
    }

    private fun replace(redis: RedisCommands<String, String>, elements: Collection<String>) {
        redis.multi()
        redis.del(hash)
        redis.lpush(hash, *elements.toTypedArray())
        redis.exec()
    }
    ...
    override fun add(element: E): Boolean {
        ...
        db.conn.sync().lpush(hash, serialized)
    ...
    override fun contains(element: E): Boolean = db.conn.sync().let(this::all).contains(serializer.serialize(element))

    override fun containsAll(elements: Collection<E>): Boolean =
        db.conn.sync().let(this::all).containsAll(elements.map(serializer::serialize))

    override fun indexOf(element: E): Int = db.conn.sync().let(this::all).indexOf(serializer.serialize(element))

    override fun lastIndexOf(element: E): Int = db.conn.sync().let(this::all).lastIndexOf(serializer.serialize(element))
    ...
    override fun toString(): String = db.conn.sync().lrange(hash, 0, -1)
        .joinToString(prefix = "[", separator = ", ", postfix = "]") { serializer(it).toString() }

    private class SubList<E>(private val list: MutableList<E>, private val fromIndex: Int, toIndex: Int) :
