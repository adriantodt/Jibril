...
open class RedisField<T>(private val serializer: Serializer<T>, private val defaultValue: T? = null) :
    ReadWriteProperty<RedisObject, T> {

    override fun getValue(thisRef: RedisObject, property: KProperty<*>): T {
        return thisRef.run {
            db.conn.sync().hget(remoteId(), property.name)?.let(serializer::unserialize) ?: defaultValue
    ...
    override fun setValue(thisRef: RedisObject, property: KProperty<*>, value: T) {
        thisRef.run {
            db.conn.sync().hset(remoteId(), property.name, serializer.serialize(value))
        }
    }
    ...
    open class Nullable<T>(private val serializer: Serializer<T?>) : ReadWriteProperty<RedisObject, T?> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): T? {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.let(serializer::unserialize)
            }
        }
    ...
    class Jackson<T : Any>(mapper: ObjectMapper, type: TypeReference<T>, defaultValue: T? = null) :
        RedisField<T>(Serializer.jackson(mapper, type), defaultValue)
    ...
    class String(private val defaultValue: kotlin.String? = null) : ReadWriteProperty<RedisObject, kotlin.String> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.String {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name) ?: defaultValue
                ?: throw IllegalStateException("Value not set.")
            }
        }
        ...
        override fun setValue(thisRef: RedisObject, property: KProperty<*>, value: kotlin.String) {
            thisRef.run {
                db.conn.sync().hset(remoteId(), property.name, value)
            }
        }
    }
    ...
    class Int(private val defaultValue: kotlin.Int? = null) : ReadWriteProperty<RedisObject, kotlin.Int> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Int {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toInt() ?: defaultValue
                ?: throw IllegalStateException("Value not set.")
            }
        }
        ...
        override fun setValue(thisRef: RedisObject, property: KProperty<*>, value: kotlin.Int) {
            thisRef.run {
                db.conn.sync().hset(remoteId(), property.name, value.toString())
            }
        }
    }
    ...
    class Boolean(private val defaultValue: kotlin.Boolean? = null) : ReadWriteProperty<RedisObject, kotlin.Boolean> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Boolean {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toBoolean() ?: defaultValue
                ?: throw IllegalStateException("Value not set.")
            }
        }
    ...
    class Long(private val defaultValue: kotlin.Long? = null) : ReadWriteProperty<RedisObject, kotlin.Long> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Long {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toLong() ?: defaultValue ?: throw IllegalStateException(
                    "Value not set."
                )
    ...
    class Double(private val defaultValue: kotlin.Double? = null) : ReadWriteProperty<RedisObject, kotlin.Double> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Double {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toDouble() ?: defaultValue
                ?: throw IllegalStateException("Value not set.")
    ...
    class NullableString : ReadWriteProperty<RedisObject, kotlin.String?> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.String? {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)
            }
        }
        ...
        override fun setValue(thisRef: RedisObject, property: KProperty<*>, value: kotlin.String?) {
            thisRef.apply {
                if (value == null) {
                    db.conn.sync().hdel(remoteId(), property.name)
                } else {
                    db.conn.sync().hset(remoteId(), property.name, value)
                }
            }
    ...
    class NullableInt : ReadWriteProperty<RedisObject, kotlin.Int?> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Int? {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toInt()
            }
        }
    ...
    class NullableBoolean : ReadWriteProperty<RedisObject, kotlin.Boolean?> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Boolean? {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toBoolean()
            }
        }
    ...
    class NullableLong : ReadWriteProperty<RedisObject, kotlin.Long?> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Long? {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toLong()
            }
        }
    ...
    class NullableDouble : ReadWriteProperty<RedisObject, kotlin.Double?> {
        ...
        override fun getValue(thisRef: RedisObject, property: KProperty<*>): kotlin.Double? {
            return thisRef.run {
                db.conn.sync().hget(remoteId(), property.name)?.toDouble()
            }
        }
